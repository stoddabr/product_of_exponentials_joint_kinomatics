...
def get_astar_path(start,goal):
    ...
    start_node.h = get_dist2d(start_node,goal_node)
    ...
    while not openlist.empty():
        ...
        if is_equal2d(cur_node,goal_node):
            goal_node.parent = cur_node
            final_path = extract_path2d(cur_node,start_node,goal_node)
            # path_cost = cur_node.g + np.sqrt((cur_node.x-goal_node.x)**2 + (cur_node.y - goal_node.y)**2 + (cur_node.theta-goal_node.theta)**2)
            return final_path

        for neighbour in get_valid_neighbours2d(cur_node):
            # neighbour is [x,y,theta]
            # print("neighbour = ",neighbour)
            if neighbour in closedlist:
                continue

            dist = np.sqrt((cur_node.x-neighbour[0])**2 + (cur_node.y-neighbour[1])**2)
            temp_g_cost = cur_node.g + dist  

            if neighbour not in openlist_nodes:

                neighbour_node = Node(neighbour,None)
                neighbour_node.g = temp_g_cost
                neighbour_node.h = get_dist2d(neighbour_node,goal_node)
                ...
                old_neigh.h = get_dist2d(old_neigh,goal_node)
